<!--
  ...
  merchant-agent/managed-agents/merchant-agent/system.md by
  runtime/scripts/derive-claude-md.py (run with --write to regenerate this file after
  system.md changes; run with no arguments to diff-check it, e.g. in a pre-push check).
  system.md is itself CMA's own derivation of
  merchant-agent/core/merchant_agent/prompt.py::build_static_system for their Managed
  Agents hosted path — see derive-claude-md.py's own docstring for why this package's
  derivation reads system.md rather than calling build_static_system directly. Patches
  on top of system.md, both applied by the script:

  * substituted: every "the preview card's approval prompt" -> "the operator portal's
    approval page" — CMA's own reference system.md was derived assuming
    stage_shows_preview=False and a platform confirmation dialog (agent.yaml's
    apply_change always_ask) doubling as the approval surface; this deployment has no
    such dialog. Approval instead happens on apps/storefront-web's own /operator page
    (D-R6 / reproduction plan §6 contract b), and stage_shows_preview stays at its
    True default — see __main__.py's _build_config comment for why. Keep this string
    in sync with __main__.py's CMA_APPROVAL_SURFACE default and agent.yaml's env.
  * appended (not present in system.md): the "Skill loading in this deployment"
    section at the very end, verbatim from CMA's own
    commerce_common.agent_sdk.SKILL_TOOL_ADAPTER — same patch as cma-shopping's
    CLAUDE.md, for the same reason (this deployment loads skills through Claude
    Code's native `Skill` tool, not system.md's assumed host-native attachment).
-->
...
- Approval happens on the operator portal's approval page, never in chat; no chip approves or applies a change.
