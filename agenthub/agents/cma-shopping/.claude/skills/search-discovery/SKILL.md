...
<!--
  ...
  shopping-agent/skills/search-discovery/SKILL.md. Apache License 2.0 (see repo
  root LICENSE / NOTICE). One light edit: the `web_search` reference in
  "Shortlist and recommendation" was removed — this deployment keeps web_search
  off by default (agent.yaml has no CMA_ENABLE_WEB_SEARCH, matching CMA's own
  default), so upstream's already-conditional "where one is registered"
  phrasing would never fire here; removed rather than left dead. Tool names
  below (present_products, present_comparison, get_product_details) refer to
  this deployment's mcp__storefront__<name> tools, delivered over the
