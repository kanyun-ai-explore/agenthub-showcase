...
def _suggestions_schema(lead: str = "") -> dict[str, Any]:
    return {
        "type": "array",
        "items": {"type": "string"},
        "minItems": 1,
        "maxItems": 4,
        "description": (
            f"{lead}1-4 chips, each a brief imperative and each a different kind "
...
def build_tools(
    ...
    presentation: list[dict[str, Any]] = [
        ...
        },
        {
            ...
            "description": (
                "Give the turn its 1-4 chips on a turn with no component (a terms answer, a "
                "clarifying question, a confirmed add or save): alone, after the text; it "
                "ends the reply. A turn with a component carries its chips in that "
                "component's own `suggestions` field instead — do not call this as well."
            ),
            "input_schema": {
                "type": "object",
                "properties": {"suggestions": _suggestions_schema()},
    ...
    # The turn's chips ride on its last component (``PresentationPayload.suggestions``)
    # so they cost no model round of their own; present_suggestions stays for the turn
    # with no component. Applied here so present_disclosure gets it too.
    for tool in tools:
        if tool["name"] in PRESENTATION_TOOL_NAMES:
            tool["input_schema"]["properties"]["suggestions"] = _suggestions_schema(
                "When this is the turn's last component, the turn's 1-4 chips, "
                "in place of a separate present_suggestions call: "
            )
