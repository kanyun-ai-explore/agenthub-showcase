...
from .presentation import CHIPS_COMPONENT, CHIPS_TOOL, PresentationComponent, enrich_partial
...
def round_closes_turn(
    ...
    calls = list(calls)
    return any(_carries_chips(name, outcome) for name, outcome in calls) and all(
        clean(*call) for call in calls
    )


def _carries_chips(name: str, outcome: ToolOutcome) -> bool:
    """The chips tool itself, or a component that carried the chips in its
    ``suggestions`` field (``run_presentation`` emits them as a second ``ui`` event)."""
    if name == CHIPS_TOOL:
        return True
    return any(
        event.type == "ui" and event.data.get("component") == CHIPS_COMPONENT
        for event in outcome.events
    )
