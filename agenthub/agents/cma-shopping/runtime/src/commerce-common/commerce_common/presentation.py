...
class PresentationPayload(BaseModel):
    ...
    ``suggestions`` is the one field every component shares: the turn's chips, carried
    on the turn's last component so they cost no model round of their own —
    :func:`run_presentation` lifts them into a second ``ui`` event on the chips
    component. Sanitized like :class:`PresentSuggestionsPayload`; a list that sanitizes
    to nothing is dropped rather than refused, since the component itself still renders.
    """

    model_config = ConfigDict(extra="ignore")

    suggestions: list[str] | None = Field(default=None, max_length=4)

    @field_validator("suggestions", mode="after")
    @classmethod
    def _sanitize_carried_suggestions(cls, chips: list[str] | None) -> list[str] | None:
        return None if chips is None else sanitize_suggestion_chips(chips)
...
async def run_presentation(
    ...
    events = [AgentEvent.ui(spec.component, enriched)]
    if spec.name != CHIPS_TOOL:
        # The carried chips are the turn's, not the component's: they leave the
        # component's payload (an enrich hook that dumps the model wholesale would
        # otherwise echo them) and become the chips component's own event.
        enriched.pop("suggestions", None)
        chips = getattr(payload, "suggestions", None)
        if chips:
            events.append(AgentEvent.ui(CHIPS_COMPONENT, {"suggestions": chips}))
    return ToolOutcome(text, events=events)
